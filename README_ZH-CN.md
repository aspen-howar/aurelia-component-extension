...
每次在添加Aurelia组件时，都需要手动添加同名的html，less和ts文件。
...
* 添加了常用基本组件代码补全，比如，输入 `<select-menu`时，会自动补全组件的属性和示例值
* 添加了组件文件跳转，比如，光标在`<select-menu>`上时，可以按`F12`快速跳转到`select-menu.html`文件 
